...
class CoinbaseClient:
    def __init__(self):
        ...
        self.base_url = (
            "https://api.exchange.coinbase.com"  # Updated URL for Advanced Trade
        )
        self.logger = self._setup_logging()

        if not self.api_key or not self.api_secret:
            self.logger.warning(
                "Using public API endpoints only - credentials not found"
            )
    ...
    def get_product_candles(
        self, product_id: str, start: str = None, end: str = None, granularity: int = 60
    ) -> List[Dict[str, Any]]:
    ...
    def place_market_order(
        self, product_id: str, side: str, size: str
        ...
        endpoint = "/orders"
        data = {"product_id": product_id, "side": side, "size": size, "type": "market"}
